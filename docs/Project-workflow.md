...
## บอร์ดวันงาน (`/jobs`) — spec: `docs/specs/day-of-board.md`

branch: `feature/day-of-board` · เริ่ม 2026-09-01 · glossary: `CONTEXT.md` (บอร์ดวันงาน, ใบงานลอย) · baseline `tsc` = 7 (checkupdate/, finance claim-detail)

| # | Ticket | Blocked by | สถานะ |
|---|---|---|---|
| 01 | /jobs กลายเป็นบอร์ดวันงาน: kanban ใบงานหน้างานอย่างเดียว + ชิปช่วงวัน (วันนี้ / 7 วันนี้ / ทั้งหมด) + แถบใบงานลอยใต้บอร์ด + เมนูเปลี่ยนชื่อเป็น "บอร์ดวันงาน" · seam `board-logic.ts` + check script | — | [x] 2026-09-01 47e0a61 — บอร์ดกราฟิกถูกยกออก (สถานะออกแบบอยู่ในพูลรายใบแล้ว), ลิงก์เดิมทุกเส้นใช้ได้เหมือนเดิม |
| 02 | เช็คอินหน้างานแล้วสถานะ "ออกหน้างาน" ขยับเอง (hook ใน `checkIn`, ActionType `AUTO_ONSITE_POOL_JOB`, ไม่ล้มการเช็คอิน) + ลิงก์ "เปิดหน้าใบงาน ↗" บนการ์ดพูลทุกแท็บ · seam `shouldAdvanceToOnsite` + check script | 01 | [x] 2026-09-01 — อ่านลำดับสถานะจาก `job_settings` ตอนรัน (แอดมินแก้ขั้นได้), ห้ามถอยหลัง, เช็คอินซ้ำไม่แตะอะไร |

### สิ่งที่ user ต้องทำเอง
- ไม่มี — รอบนี้ไม่แตะ schema และไม่มี migration ใหม่
