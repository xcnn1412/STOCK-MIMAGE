...
export default function EditEventForm({
  ...
  // Parse existing staff/seller from comma-separated string
  const parseNames = (str: string | null | undefined): string[] => {
    if (!str) return []
    return str.split(',').map(s => s.trim()).filter(Boolean)
  }

  // Multi-select state for staff and seller
  const [selectedStaff, setSelectedStaff] = useState<string[]>(parseNames(event.staff))
  const [selectedSellers, setSelectedSellers] = useState<string[]>(parseNames((event as any).seller))
