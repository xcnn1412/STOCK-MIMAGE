...
import {
  Plus, Search, Filter, LayoutGrid, List, ChevronDown, AlertCircle, Tag, Calendar, CalendarClock, MessageSquare, X
} from 'lucide-react'
...
export default function CrmDashboard({ leads, settings, users }: CrmDashboardProps) {
  ...
  const filteredLeads = useMemo(() => {
    return leads.filter(lead => {
      ...
      if (eventFrom || eventTo) {
        ...
        if (eventFrom && lead.event_date < eventFrom) return false
        if (eventTo && lead.event_date > eventTo) return false
      }
      if (search) {
  ...
  }, [leads, statusFilter, sourceFilter, saleFilter, tagFilter, createdFrom, createdTo, eventFrom, eventTo, search])
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <DateRangeFilter
            ...
            from={eventFrom}
            to={eventTo}
            onChange={(f, t) => { setEventFrom(f); setEventTo(t) }}
            locale={locale}
          />
...
// ============================================================================
// Date Range Filter (Popover with presets)
// ============================================================================
...
function DateRangeFilter({ icon, labelTh, labelEn, from, to, onChange, locale }: DateRangeFilterProps) {
