...
export default async function SalesBoardPage() {
  const [leads, claims, installments, jobEvents, costItems, events, eventStaff, profiles] = await Promise.all([
    ...
    fetchAll('events', 'id, event_date'),
    fetchAll('event_staff', 'event_id, user_id, created_at'),
  ...
  return (
    <SalesBoardView
      ...
      events={events as never} eventStaff={eventStaff as never} profiles={profiles as never}
    />
  )
}
